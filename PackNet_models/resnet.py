...
__all__ = ['GEMResNet18', 'ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101',
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        w = m.weight * m.mask
        out = F.conv2d(
            x, w, m.bias, m.stride, m.padding, m.dilation, m.groups
        )
        out = self.bn1(out)
        out = self.relu(out)
        ...
        w = m.weight * m.mask
        out = F.conv2d(
            out, w, m.bias, m.stride, m.padding, m.dilation, m.groups
        )
        out = self.bn2(out)
...
def GEMResNet18(num_classes, width_per_group=20, affine=False) -> GEMResNet:
    global AFFINE
    AFFINE = affine
